...
fn isBoolObject(py_obj: ?*ffi.PyObject) !bool {
    const py_false = ffi.PyBool_FromLong(0) orelse return PyError.PyRaised;
    defer ffi.Py_DecRef(py_false);
    if (py_obj == py_false) return true;

    const py_true = ffi.PyBool_FromLong(1) orelse return PyError.PyRaised;
    defer ffi.Py_DecRef(py_true);
    return py_obj == py_true;
}
