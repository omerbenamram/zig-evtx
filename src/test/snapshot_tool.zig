...
const snapshot_tests = @import("snapshot_tests.zig");
...
fn getProjectRoot() []const u8 {
    // This file is at src/test/snapshot_tool.zig
    // Project root is two directories up
    const src_path = @src().file;
    // Find the last two slashes and return everything before
    var path = src_path;
    var count: usize = 0;
    var i = path.len;
    while (i > 0) : (i -= 1) {
        if (path[i - 1] == '/') {
            count += 1;
            if (count == 3) { // src/test/file.zig = 3 components
                return path[0 .. i - 1];
            }
        }
    }
...
pub fn main() !void {
    ...
    try stdout.writeAll("Running snapshot tests...\n\n");
        ...
        switch (result.result) {
            ...
            .fail => {
                ...
                if (result.message) |msg| {
                    ...
                    try stdout.writeAll("Output differs\n");
                }
                ...
                if (result.actual != null and result.expected != null) {
                    ...
                    while (true) {
                        ...
                        if (!std.mem.eql(u8, exp, act)) {
                            try stdout.print("        First diff at line {d}:\n", .{line_num});
            ...
            .@"error" => {
                ...
                if (result.message) |msg| {
                    ...
                    try stdout.writeAll("Unknown error\n");
                }
                failed += 1;
            },
        }
    ...
    try stdout.print("Results: {d} passed, {d} failed", .{ passed, failed });
    if (skipped > 0) {
        try stdout.print(", {d} skipped", .{skipped});
    }
    try stdout.writeAll("\n");
